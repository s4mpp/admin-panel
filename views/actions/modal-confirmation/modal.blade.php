<div class="flex mt-5 justify-end items-center gap-4">
	...
	<x-button x-on:click="{{ $action->getNameModalConfirmation() }} = false, {{ $action->getNameModal() }} = true" className="{{ $action->isDangerous() ?  'bg-red-500 hover:bg-red-600 text-white' : 'btn-primary' }}">Continuar</x-button>
</div>